...
def manipulator(
    filename: str, target_positions: list[np.ndarray], is_save: bool = False
):
    ...
    if hand_l is None:
        print("not found hand_l")
        return
    hand_r = robot.get_link("right_hand_default")
    if hand_r is None:
        print("not found hand_r")
        return

    target_tfs = [Transform(p) for p in target_positions]
    bt = time.time()
    ik_result = robot.ik_solve([hand_l, hand_r], target_positions, robot.mid_position())
    ft = time.time()
    ...
    for i, t in enumerate(target_positions):
        ax.scatter(t[0], t[1], t[2], s=80, label=f"target {i:02d}")
    ax.legend()
...
def main():
    ...
    default_urdf = os.path.join(
        os.path.dirname(__file__), "../tests/files/human165.urdf"
    )
    ...
    targets = [np.array([0.25, 0.1, 0.8]), np.array([0.25, -0.1, 0.8])]

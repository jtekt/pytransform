...
def manipulator(target_position: np.ndarray, is_save: bool = False):
    ...
    # origin
    base_link = Transform(position=np.array([0.0, 0.0, 0.0]), name="base_link")

    # define manipulator structure
    arm_base = Transform(position=np.array((0.0, 0.0, 0.0)), name="arm_base")

    link1 = Transform(position=np.array((0.0, 0.0, 2.0)), name="link1")

    link2 = Transform(position=np.array((0.0, 0.0, 4.0)), name="link2")

    link3 = Transform(position=np.array((0.0, 0.0, 6.0)), name="link3")

    end_effector = Transform(position=np.array((0.0, 0.0, 8.0)), name="end_effector")
    ...
    ik_result = robot_arm.ik_solve(
        [robot_arm.links[-1]],
        [target_position],
        initial_position=robot_arm.mid_position(),
    )
    ft = time.time()
    ...
    ax.scatter(target_position[0], target_position[1], target_position[2], s=80)
